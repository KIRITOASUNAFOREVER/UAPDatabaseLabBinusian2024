...
-- Jawaban Nomor 1
SELECT PaintingID, PaintingName, PaintingPrice FROM Painting
WHERE LEN(PaintingName) > 15

-- Jawaban Nomor 2
SELECT HT.TransactionID, MC.CustomerName, HT.TransactionDate FROM Customer MC
JOIN HeaderTransaction HT ON MC.CustomerID = HT.CustomerID
WHERE DATENAME(MONTH,HT.TransactionDate) = 'September'

-- Jawaban Nomor 3
SELECT P.PaintingName, 'IDR ' + CAST(P.PaintingPrice AS VARCHAR) + ',-' AS [Price], 'IDR ' + CAST(SUM(DT.Quantity * P.PaintingPrice) AS VARCHAR) + ',-' AS [Total Profit]
FROM Painting P JOIN DetailTransaction DT ON P.PaintingID = DT.PaintingID
WHERE P.PaintingName LIKE '%d%'
GROUP BY P.PaintingName, P.PaintingPrice

-- Jawaban Nomor 4
SELECT C.CustomerName, HT.TransactionID, HT.TransactionDate, COUNT(HT.TransactionID) AS [Total of Painting], SUM(DT.Quantity) AS [Total of Quantity] FROM Customer C 
JOIN HeaderTransaction HT ON C.CustomerID = HT.CustomerID 
JOIN DetailTransaction DT ON HT.TransactionID = DT.TransactionID
WHERE YEAR(C.CustomerDOB) % 2 = 1
...
-- Jawaban Nomor 5 (Versi Tanpa SubQuery)
SELECT MC.CustomerName, MC.CustomerAddress, MC.CustomerEmail, CONVERT(NVARCHAR(30),MC.CustomerDOB,106) AS [Date of Birth] FROM Customer MC
JOIN HeaderTransaction HT ON MC.CustomerID = HT.CustomerID
JOIN DetailTransaction DT ON HT.TransactionID = DT.TransactionID
JOIN Painting MP ON MP.PaintingID = DT.PaintingID
JOIN PaintingType PT ON PT.PaintingTypeID = MP.PaintingTypeID
WHERE DATENAME(MONTH, HT.TransactionDate) = 'July' AND PT.PaintingTypeName IN('Abstract')

-- Jawaban Nomor 5 (Versi SubQuery)
SELECT C.CustomerName, C.CustomerAddress, C.CustomerEmail, CONVERT(VARCHAR, C.CustomerDOB, 106) AS [Date of Birth] FROM Customer C 
JOIN HeaderTransaction HT ON C.CustomerID = HT.CustomerID 
JOIN DetailTransaction DT ON HT.TransactionID = DT.TransactionID 
JOIN Painting P ON P.PaintingID = DT.PaintingID 
JOIN PaintingType PT ON PT.PaintingTypeID = P.PaintingTypeID
WHERE HT.TransactionDate IN (
	SELECT TransactionDate
	FROM HeaderTransaction
	WHERE DATENAME(MONTH, TransactionDate) LIKE 'July'
) AND PT.PaintingTypeName LIKE 'Abstract'

-- Jawaban Nomor 6
SELECT DISTINCT C.CustomerID, C.CustomerName, C.CustomerPhoneNumber, C.CustomerEmail FROM Customer C 
JOIN HeaderTransaction HT ON C.CustomerID = HT.CustomerID 
JOIN DetailTransaction DT ON HT.TransactionID = DT.TransactionID, (
	SELECT AVG(Quantity) AS [AverageQuantity]
	FROM DetailTransaction
) AS A
WHERE CHARINDEX(' ', C.CustomerName, 1) != 0 AND DT.Quantity > A.AverageQuantity
ORDER BY C.CustomerName ASC

-- Jawaban Nomor 7
CREATE VIEW [StaffOddView] AS
SELECT 'Staff ' + RIGHT(MS.StaffID,3) AS [StaffID], MS.StaffName, MS.StaffAddress FROM Staff MS
WHERE CONVERT(INT, RIGHT(MS.StaffID,3)) % 2 = 1

SELECT * FROM StaffOddView

-- Jawaban Nomor 8
CREATE VIEW [TransactionDataOfModernismPainting] AS
SELECT HT.TransactionID, DATENAME(DW,HT.TransactionDate) AS [Day of Transaction], HT.TransactionDate, UPPER(MP.PaintingName) AS [Painting Title], SUM(DT.Quantity) AS [Quantity] FROM HeaderTransaction HT
JOIN DetailTransaction DT ON HT.TransactionID = DT.TransactionID
JOIN Painting MP ON MP.PaintingID = DT.PaintingID
JOIN PaintingType PT ON PT.PaintingTypeID = MP.PaintingTypeID
WHERE PT.PaintingTypeName LIKE 'Modernism'
GROUP BY HT.TransactionID,HT.TransactionDate, MP.PaintingName

SELECT * FROM TransactionDataOfModernismPainting
...
ALTER TABLE Customer
ADD CONSTRAINT chk_social_media CHECK(CustomerSocialMedia BETWEEN 5 AND 25)
...
DELETE Customer FROM Customer C 
JOIN HeaderTransaction HT ON C.CustomerID = HT.CustomerID 
JOIN DetailTransaction DT ON HT.TransactionID = DT.TransactionID
